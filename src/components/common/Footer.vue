<template>
    <div>
        <!-- Footer -->
        <footer class="relative z-50 bg-foot text-gray-600 text-sm py-4 px-5/100 text-center border-t border-gray-200">
            <div class="flex flex-col items-center space-y-4 md:flex-row md:justify-between md:space-y-0">
                <p>© 2024 LIANG YI</p>
                <nav class="hidden md:flex space-x-6">
                    <a
                        href="javascript:void(0)"
                        v-for="item in Menu" :key="item.name" :href="item.href"
            ...
            <div class="absolute -z-10 w-full h-10 -top-10 left-0 overflow-hidden">
                <button
                    v-show="showButton"
                    class="block absolute left-1/2 -translate-x-1/2 -bottom-8 w-16 h-16 bg-foot rounded-full border border-foot  shadow-defaultBox text-gray-600 transition hover:scale-110 hover:text-black"
                    @click="scrollToTop"
